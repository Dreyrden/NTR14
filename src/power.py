...
def calculate_power(mission):
   ...
   '''
    ...
    system_power = 0.0
    system_mass = 0.0
    mission.power += system_power

    orbital_parameter = 3.986E14
    earth_radius = 6378E3

    orbital_period = 2*math.pi()*math.pow(math.pow(mission.ntr.sma, 3)/orbital_parameter,1/2)
    eclipse_period = (orbital_period*math.asin(earth_radius/mission.ntr.sma))/180

    # Assuming Peak power tracking regulation scheme Xe = 0.6, Xd = 0.8
    Xe = 0.6
    Xd = 0.8
    solar_panel_power = (mission.power*eclipse_period/Xe + mission.power*(orbital_period-eclipse_period)/Xd)/Xd
    ...
    mission.power.mass += array_mass + system_mass
